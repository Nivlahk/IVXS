...
const { IVXWorkerPool, specFromResolved } = __ivxRequire('./ivx_workers.js', 'Workers');
...
// Page-level singleton, mirroring IVX.resolver. runtime.js's Import
// wiring reaches for window.IVX.bridge directly rather than constructing
// its own -- one shared worker pool for the whole page, not one per import.
//
// BUG, found from a live page: this used to set ONLY window.IVX.Bridge.bridge
// (nested under the capitalized export), never the top-level window.IVX.bridge
// that runtime.js actually checks (`!window.IVX.bridge`). ivx_resolve.js sets
// BOTH window.IVX.Resolver (class) and window.IVX.resolver (instance) as two
// separate top-level assignments -- ivx_bridge.js only ever did the first
// half of that pattern. Console check that caught it: `true true false` for
// (IVX exists, resolver exists, bridge exists) -- resolver was wired right,
// bridge was not.
let __ivxDefaultResolver = null;
try { __ivxDefaultResolver = __ivxRequire('./ivx_resolve.js', 'Resolver').resolver; }
catch (_) { /* resolver not loaded yet at this point in script order -- fine, resolve() will fail loudly per-call instead */ }
const __ivxDefaultBridge = new IVXBridge({ resolver: __ivxDefaultResolver });
__ivxExport('Bridge', { IVXBridge, isHoistable, materializeInt, bridge: __ivxDefaultBridge });
__IVX_ROOT.IVX.bridge = __ivxDefaultBridge;   // top-level, matching IVX.resolver's pattern
