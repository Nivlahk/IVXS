...
## Browser integration (index.html)

**This did not exist until now.** Phases 1–4 produced data and nothing in the
page consumed it, so `index.html` went untouched for four phases. Worse, the
modules were **CommonJS-only** — five of the six threw `require is not
defined` or `module is not defined` on the first line of a real page load.
They were never browser-loadable at all. Verified in a clean context before
fixing.

### What changed

- **All modules are dual-mode**, wrapped in an IIFE and exporting onto
  `window.IVX`. The IIFE matters: classic scripts share one global scope, so
  top-level `const` in two files collides with *"Identifier has already been
  declared"* and kills the page.
- **`word_kh.js` too.** It only *appeared* to work before, because function
  declarations hoist — `wordParse` got defined and then the last line threw,
  leaving a console error on every load.
- **`ivx_lens_ui.js`** (new) — the lens panel.
- **`index.html`** — 71 added lines, **zero deletions**. Inline `<style>`
  block, the `#lens-panel` markup inside `#ep-body`, and nine `<script>` tags
  in dependency order.

### The lens panel is a projection, not an edit

It never writes to `#src`. The existing `materializeToggle` path overwrites
the textarea with lowered text and unchecks the toggles, so the high-altitude
form is gone and there is no way back. This panel renders a separate view
driven by the source map, so switching altitude is reversible — which is the
entire reason the map exists.

Three altitudes: **Source** (your line → what it cost), **IR** (CF
instructions, each traced to a source line), **Machine** (PC, bytes,
cycles). "Hide scaffolding" drops instructions with no counterpart in your
source, so CN setup and the boot prefix are visible but never attributed to
you.

### Verified in a real DOM

Run under jsdom with real `<script>` tags — the first time any of this has
executed in a browser environment rather than through a shim. All nine files
load, all three altitudes render:

```
[source] 11 rows | 11 source construct(s), 39 CF lines
[cf]     14 rows | 14 shown, 25 synthetic hidden
[flat]   21 rows | 75 instructions, 279 bytes, 81 cycles
                 | 46 traced to source, 29 engine-synthesized
```

It caught a bug a shim never would: **`const` at the top level of a
`<script>` is a lexical binding that never becomes a `window` property.** The
engine declares `STACK_CAP_IDX`/`GEN`/`MAC`/`SIZE` that way, so reading them
off `window` gave `undefined` and the ABI prologue threw *"Cannot mix BigInt
and other types"* from deep inside the lowerer. Indirect `eval` runs in
global scope and can see them; the panel now reads them that way and reports
the gap explicitly if they're unreachable.

### ⚠ The engine and index.html cannot share a page as-is
...
`materializeToggle`, `imemBudgetSlider`, and so on. Its init code will throw
on load if you paste it into `index.html`.

The lens degrades honestly here: Source and IR altitudes need only the
lowerer and work without the engine; Machine reports exactly which functions
are missing. But to get the Machine altitude on `nivlahk.com` you need one
of:

1. keep the engine as its own page and load the lens there;
2. add the 40 elements to `index.html` (hidden is fine);
3. guard the engine's init so it skips elements that aren't present.

(3) is the smallest change and the one I'd pick, but it edits the engine's
init path, so it's your call — I didn't want to make that decision inside a
file you run in production.
...
- **The engine/index.html DOM mismatch above** — one of the three options
  has to be picked before the Machine altitude works on the live site.
- **Phases 1 and 4 have no UI.** `ivx_resolve` / `ivx_workers` / `ivx_bridge`
  load and are reachable as `window.IVX.*`, but nothing in the page calls
  them — no import panel, no worker status strip.
- **`onZoningToggleChange` is untouched.** The old destructive materialize
  path still exists alongside the new panel.
