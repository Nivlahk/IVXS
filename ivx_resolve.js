...
// ────────────────────────────────────────────────────────────────────────────
// 6b. RELATIVE DEPENDENCY WALKING (for URL imports)
// ────────────────────────────────────────────────────────────────────────────
// A bare URL fetches exactly ONE file. Most real source files in a repo
// require their siblings, so `.../lodash/isEmpty.js` resolved fine and then
// died in the worker on require('./_baseKeys'). The npm path never hit this
// because it unpacks a whole tarball.
//
// This walks relative requires from a URL entry point, fetching each sibling
// and keying it by its path relative to the entry's directory -- the same
// keys the worker's require() shim resolves against.
//
// Bounded on purpose. A source tree can be enormous and every hop is a
// network round trip, so this caps total files and refuses to escape above
// the entry's own directory tree. Hitting a cap is REPORTED, not silently
// truncated: a partial module map fails later, deep inside the worker, with
// a much worse error than "dependency limit reached".
const DEP_LIMITS = { maxFiles: 300, maxDepth: 12 };

// Matches require('./x') / require("../x"). Deliberately only RELATIVE ids:
// a bare require('lodash') is a different package and is not followed.
...
/**
 * Fetches an entry URL's relative dependency graph.
 * @returns {{ moduleMap: Object, entryPath: string, fetched: number, truncated: string|null }}
 */
async function walkRelativeDeps(entryUrl, entrySource, opts = {}) {
  const limits = { ...DEP_LIMITS, ...(opts.limits || {}) };
  // Injectable so callers (and tests) need not mutate a shared global.
  // Tests that assigned global.fetch concurrently stomped each other's
  // mocks and reported failures that had nothing to do with this code.
  const doFetch = opts.fetchImpl || ((...a) => fetch(...a));
  const entryUrlObj = new URL(entryUrl);
  const entryFile = entryUrlObj.pathname.split('/').pop() || 'index.js';
  const baseDirUrl = new URL('.', entryUrlObj);   // entry's directory

  const moduleMap = {};
  const seen = new Set();
  let truncated = null;

  // Candidate suffixes, mirroring the worker's resolution order.
  const candidatesFor = (p) => [p, p + '.js', p + '.json', p + '/index.js'];

  async function fetchOne(relPath) {
    // Try each candidate until one returns 200.
    for (const cand of candidatesFor(relPath)) {
      if (Object.prototype.hasOwnProperty.call(moduleMap, cand)) return cand;
      const url = new URL(cand, baseDirUrl).href;
      let res;
      try { res = await doFetch(url); } catch (_) { continue; }
      if (!res.ok) continue;
      moduleMap[cand] = await res.text();
      return cand;
    }
    return null;
  }

  function relativeIdsIn(src) {
    const ids = new Set();
    let m;
    RELATIVE_REQUIRE_RE.lastIndex = 0;
    while ((m = RELATIVE_REQUIRE_RE.exec(src)) !== null) ids.add(m[1]);
    return ids;
  }

  // Breadth-first, fetching each LEVEL in parallel. The first version walked
  // depth-first one file at a time: lodash's cloneDeep pulled 108 files in
  // ~17s of purely sequential round trips, which is a miserable first-import
  // wait in a browser. Same files, same bounds -- just not one at a time.
  moduleMap[entryFile] = entrySource;
  seen.add(entryFile);

  let frontier = [entryFile];
  for (let depth = 0; depth <= limits.maxDepth && frontier.length; depth++) {
    // Collect every unseen relative target across the whole current level.
    const targets = [];
    for (const fromPath of frontier) {
      const src = moduleMap[fromPath];
      if (src === undefined) continue;
      for (const id of relativeIdsIn(src)) {
        const target = normalizePath(dirnameOf(fromPath) + '/' + id);
    ...
    const room = limits.maxFiles - Object.keys(moduleMap).length;
    if (room <= 0) { truncated = truncated || `file limit ${limits.maxFiles} reached`; break; }
    const batch = targets.slice(0, room);
    if (batch.length < targets.length) {
      truncated = truncated || `file limit ${limits.maxFiles} reached`;
    }

    const resolvedKeys = await Promise.all(batch.map(t => fetchOne(t)));
    frontier = resolvedKeys.filter(Boolean);
    // A dependency that 404s is NOT fatal: it may sit in a branch that never
    // executes. The worker raises a precise error naming the id if it is
    // actually needed at runtime.

    if (depth === limits.maxDepth && frontier.length) {
      truncated = truncated || `depth limit ${limits.maxDepth} reached`;
    }
  }
  ...
  return {
    moduleMap,
    entryPath: entryFile,
    fetched: Object.keys(moduleMap).length,
    truncated,
  };
}

/**
 * Walks a Python entry file's SIBLING imports. Deliberately much narrower
 * than the JS walker: Python's import system has packages, namespace
 * packages, relative-vs-absolute resolution and stdlib shadowing, none of
 * which can be settled from URLs alone. This only handles the case that
 * actually comes up for a single file grabbed off GitHub -- other .py files
 * sitting next to it.
 *
 * Stdlib and third-party imports are skipped for free: if `<name>.py` does
 * not exist next to the entry file, the fetch 404s and it is left alone for
 * Pyodide's own import machinery to resolve (which handles stdlib and any
 * micropip-installed package correctly).
 */
async function walkPythonDeps(entryUrl, entrySource, opts = {}) {
  const limits = { ...DEP_LIMITS, ...(opts.limits || {}) };
  const doFetch = opts.fetchImpl || ((...a) => fetch(...a));
  const entryUrlObj = new URL(entryUrl);
  const baseDirUrl = new URL('.', entryUrlObj);

  const files = {};          // 'name.py' -> source
  const seen = new Set();
  let truncated = null;

  // `import foo`, `import foo as f`, `from foo import x`, `from .foo import x`
  const IMPORT_RE = /^\s*(?:import\s+([A-Za-z_]\w*)|from\s+\.?([A-Za-z_]\w*)\s+import)/gm;
  ...
  async function fetchSibling(modName) {
    const url = new URL(modName + '.py', baseDirUrl).href;
    let res;
    try { res = await doFetch(url); } catch (_) { return null; }
    if (!res || !res.ok) return null;
    return res.text();
  }

  const entryName = (entryUrlObj.pathname.split('/').pop() || 'module.py').replace(/\.py$/, '');
  files[entryName + '.py'] = entrySource;
  seen.add(entryName);

  let frontier = [entrySource];
  for (let depth = 0; depth <= limits.maxDepth && frontier.length; depth++) {
    const wanted = [];
    for (const src of frontier) {
      for (const n of importedNames(src)) {
    ...
    if (Object.keys(files).length + wanted.length > limits.maxFiles) {
      truncated = truncated || `file limit ${limits.maxFiles} reached`;
      break;
    }
    const fetched = await Promise.all(wanted.map(async n => ({ n, src: await fetchSibling(n) })));
    frontier = [];
    for (const { n, src } of fetched) {
      if (src === null) continue;    // not a sibling -- stdlib or third-party, leave to Pyodide
      files[n + '.py'] = src;
      frontier.push(src);
    }
  }

  return { files, entryName, fetched: Object.keys(files).length, truncated };
}
...
class IVXResolver {
  ...
  async _resolveUrl(cls, evidence, opts = {}) {
    const url = cls.url;
    opts = { ...this.opts, ...opts };
    ...
    // For JS, follow relative requires so a single-file URL from a real
    // repo works rather than dying in the worker on require('./sibling').
    let moduleMap = null, entryPath = null;
    if (runtime === RUNTIME.JS && opts.followDeps !== false) {
      try {
        const walked = await walkRelativeDeps(url, text, { limits: opts.depLimits });
        if (walked.fetched > 1) {
          moduleMap = walked.moduleMap;
          entryPath = walked.entryPath;
          evidence.push({ step: 'relative-deps', fetched: walked.fetched,
                          truncated: walked.truncated });
          if (walked.truncated) {
            notes.push(`dependency walk stopped early: ${walked.truncated}. `
              + `A require() for an unfetched file will fail at call time.`);
          }
        }
      } catch (e) {
        notes.push(`relative dependency walk failed (${e.message}); `
          + `this module will only work if it is self-contained`);
      }
    }
...
GLOBAL.IVX.walkPythonDeps = walkPythonDeps;
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { IVXResolver, RUNTIME, KIND, classifySpecifier, sniffBytes, resolver: GLOBAL.IVX.resolver,
                     walkRelativeDeps, walkPythonDeps, normalizePath, dirnameOf, DEP_LIMITS,
