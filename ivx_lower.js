...
const { CFEmitter, REFUSAL_CODES, FIDELITY } = __ivxRequire('./ivx_lower_contract.js', 'LowerContract');
...
// Location for an expression-level span: its own if the parser stamped one,
// otherwise the enclosing statement's.
function exprLoc(lowerer, node) {
  const own = locOf(node);
  if (own.line != null) return own;
  return lowerer.stmtLoc || own;
}
...
class Lowerer {
  ...
  stmt(node) {
    ...
    // Expression nodes are not line-stamped by the parser (only statements
    // are), so a Call inside an assignment had no source line and showed up
    // as an unattributed "<Call>" row in the lens. Carry the statement's
    // location down for any span an expression opens.
    this.stmtLoc = locOf(node);
    switch (node.type) {
  ...
  callExpr(node) {
    ...
    this.e.begin(id, 'Call', exprLoc(this, node), 'abi');
    ...
    this.e.begin(id, 'Call', exprLoc(this, node), 'body');
...
__ivxExport('Lower', { lowerUAST, Lowerer, SEER_OPS, checkOpTable, CMP, ARITH,
