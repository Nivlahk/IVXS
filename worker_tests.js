...
(async () => {
  ...
  console.log('\n[1] js runtime');
  {
    ...
    await check("a BARE require() is refused, and says why", async () => {
      // Relative requires ARE now followed (the resolver walks them into a
      // moduleMap); bare package ids deliberately are not -- that would be
      // a second package, not part of this module.
      const v = new IVXWorker({ id: 'req' });
      await rejects(v.init({ runtime: 'js', source: 'const x=require("fs");module.exports={}' }),
        'Bare package requires are not followed', 'bare require');
      v.terminate();
    });

    await check('a relative require resolves from the supplied moduleMap', async () => {
      const v = new IVXWorker({ id: 'relreq' });
      await v.init({
        runtime: 'js',
        source: 'const helper = require("./helper");module.exports={ doubled: (n) => helper.twice(n) };',
        entryPath: 'index.js',
        moduleMap: { 'index.js': '', 'helper.js': 'module.exports={ twice: n => n*2 };' },
      });
      eq(await v.call('doubled', [21]), 42, 'relative require');
      v.terminate();
    });

    await check('a CIRCULAR relative require terminates (node semantics)', async () => {
      // The cache entry is installed before the body runs, so a cycle sees a
      // partially populated exports object instead of recursing forever.
      // Real packages contain cycles -- lodash does.
      const v = new IVXWorker({ id: 'cycle' });
      await v.init({
        runtime: 'js',
        source: 'const a = require("./a");module.exports={ go: () => a.name() };',
        entryPath: 'index.js',
        moduleMap: {
          'index.js': '',
          'a.js': 'const b = require("./b");module.exports.name = () => "a+" + b.tag();',
          'b.js': 'const a = require("./a");module.exports.tag = () => "b";',
        },
      });
      eq(await v.call('go', []), 'a+b', 'circular require');
      v.terminate();
    });

    await check('an unresolvable relative require names the file and the count', async () => {
      const v = new IVXWorker({ id: 'missing' });
      await rejects(v.init({
        runtime: 'js',
        source: 'require("./nope");module.exports={};',
        entryPath: 'index.js',
        moduleMap: { 'index.js': '' },
      }), 'was not among them', 'missing relative dep');
      v.terminate();
    });
    w.terminate();
  }
  ...
  console.log('\n[3b] input() -- genuine synchronous blocking via SharedArrayBuffer + Atomics');
  {
    // Same shim as pytest2.js: fake just enough of a browser worker global
    // scope (self/importScripts) to satisfy the host-detection check, while
    // running for real inside a genuine worker_threads OS thread -- so
    // Atomics.wait below is REAL blocking, not simulated. This does not
    // depend on real Pyodide's internals; it proves the mechanism itself.
    const { Worker: RealWorker } = require('worker_threads');
    const FAKE_PYODIDE_INPUT = `
      function loadPyodide() {
        ...
        return Promise.resolve({
          ...
          setStderr: () => {},
          globals: { get: n => g.get(n), set: (n, v) => g.set(n, v) },
          runPython: function (src) {
            if (src.includes('builtins.input')) { g.set('input', g.get('__ivx_sync_input')); return; }
            const m = /^guess = int\\(input\\("([^"]*)"\\)\\)$/.exec(src.trim());
            if (m) {
              const val = g.get('input')(m[1]);
              if (stdoutCb) stdoutCb('got: ' + val);
              return;
            }
          },
        });
    ...
    const BROWSER_SHIM = `
      var self = { postMessage: m => require('worker_threads').parentPort.postMessage(m) };
      Object.defineProperty(self, 'onmessage', { set: fn => {
        require('worker_threads').parentPort.on('message', m => fn({ data: m }));
      }});
      function importScripts() {}`;
    const fullSource = BROWSER_SHIM + '\n' + FAKE_PYODIDE_INPUT + '\n'
      + require(path.join(HERE, 'ivx_worker_runtime.js')).IVX_WORKER_SOURCE;

    function send(w, msg, seqRef, timeoutMs = 8000) {
      return new Promise((resolve, reject) => {
        const id = ++seqRef.n;
        const t = setTimeout(() => reject(new Error(`timeout on ${msg.op}`)), timeoutMs);
        const onMsg = m => {
          if (m.op === 'input-request') return;
          if (m.id !== id) return;
          clearTimeout(t); w.off('message', onMsg);
    ...
    await check('a real OS thread blocks on Atomics.wait until answered', async () => {
      const w = new RealWorker(fullSource, { eval: true });
      const seqRef = { n: 0 };
      const inputBuffer = new SharedArrayBuffer(8 + 4096);
      const flags = new Int32Array(inputBuffer, 0, 2);
      const payload = new Uint8Array(inputBuffer, 8);
      function provideInput(text) {
      ...
      try {
        await new Promise(r => w.once('message', r));   // 'ready' hello
        await send(w, { op: 'init', runtime: 'python', indexURL: 'https://x/', inputBuffer }, seqRef);

        let requestedPrompt = null;
        w.on('message', m => {
          if (m.op !== 'input-request') return;
          requestedPrompt = m.prompt;
          setTimeout(() => provideInput('42'), 400);
        });

        const t0 = Date.now();
        const result = await send(w, { op: 'eval', source: 'guess = int(input("Enter your guess: "))' }, seqRef);
        const elapsed = Date.now() - t0;

        if (requestedPrompt !== 'Enter your guess: ') throw new Error(`prompt text wrong: ${requestedPrompt}`);
        if (!result || !result.stdout || result.stdout[0] !== 'got: 42') {
          throw new Error(`wrong result: ${JSON.stringify(result)}`);
        }
        // The whole point: this took as long as the delay, proving a real
        // thread block-and-wake, not a race or a stub that returns instantly.
        if (elapsed < 380) throw new Error(`round trip was ${elapsed}ms -- too fast to be real blocking`);
      } finally { w.terminate(); }
    });
  }
