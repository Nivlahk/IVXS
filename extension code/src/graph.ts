...
export function validNodeIO(nodes: any[], edges: any[]): string[] {
    const errors: string[] = [];
...
export function isIvx(doc: vscode.TextDocument): boolean {
  return doc.languageId === 'kh' || doc.fileName.endsWith('.kh') || doc.fileName.endsWith('.ivx');
}

export function buildGraph(doc: vscode.TextDocument, options?: any): { graph: any, errors: string[] } {
  const graph  = parsekh(doc.getText());
  const errors = (options?.validate ?? true) ? (graph.validationErrors ?? []) : [];
  return { graph, errors };
}

export function createDiagnostics(doc: vscode.TextDocument, errors: string[]): vscode.Diagnostic[] {
