// Wrapped in an IIFE, matching every other IVX file. Node/wordLex/
// WordParser/wordParse were previously bare top-level declarations --
// classic <script> tags share one global scope, and word_kh.js was
// explicitly written to mirror the real KH parser's internal structure,
// so a name as generic as `Node` colliding with something in core.js (a
// `class`/`const`/`let` Node, in particular) is a real risk, not a
// theoretical one: reproduced with `class Node {}` in one script tag and
// this file's original `function Node` in another -- the SECOND script
// throws `Identifier 'Node' has already been declared` before a single
// line of it runs, wordParse never gets defined, and
// ivx_lens_ui.js's "word_kh.js is not loaded" message is the visible
...
// Dual-mode: index.html loads this as a classic <script>, where
// `module` does not exist. It only *appeared* to work before because
// function declarations hoist -- wordParse was defined and then this line
// threw, leaving an error on every page load.

;(function () {
  const ROOT = (typeof globalThis !== 'undefined' ? globalThis : window);
  ROOT.IVX = ROOT.IVX || {};
  ROOT.IVX.wordParse = wordParse;
  ROOT.wordParse = wordParse;
  if (typeof module !== 'undefined' && module.exports) module.exports = { wordParse };
})();
